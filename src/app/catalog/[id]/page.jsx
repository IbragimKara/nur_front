  ...
  return (
   <div className={s.page}>
   {
    ...
    <div className={s.page__buttons}>
      {
        Number(params.id) <= 0 ?null
        : <Link className={s.page__button} href={'/catalog/' + (Number(params.id) - 1)}>{params.id}</Link>
      }
     
      <div className={s.page__text}>{Number(params.id) + 1}</div>
      <Link className={s.page__button} href={'/catalog/' + (Number(params.id) + 1)}>{Number(params.id) + 2}</Link>
    </div>
    </div>
  </div>
